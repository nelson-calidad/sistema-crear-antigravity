const SPREADSHEET_ID = '1BgP_FlLmoSAPZHvTvJ746w8LmDisaVX78XjotOW_UHo';
...
const ACTIVITY_CONFIG_DEFAULTS_ = [
  ['PERIODO_ACTIVO', '', 'ID del período activo'], ['TOTAL_PUNTOS_PERIODO', '100', 'Puntos objetivo inicial'], ['PERMITIR_REASIGNACION', 'Sí', 'Permite reasignar actividades'], ['REQUERIR_EVIDENCIA', 'No', 'Solicita evidencia al completar'], ['MOSTRAR_PUNTOS_TARJETA', 'No', 'Los puntos no se muestran en el tablero'], ['MODO_CALCULO_PORCENTAJE', 'PUNTOS_OBTENIDOS', 'Cálculo de cumplimiento'],
];
...
function activityValidate_(record, founders, periods) {
  ...
  if (activityNumber_(record.PUNTOS) <= 0) throw new Error('Los puntos deben ser mayores que cero.');
  ...
  if (activityText_(record.ESTADO) === 'Completada' && !activityText_(record.EVIDENCIA).trim() && !activityText_(record.OBSERVACIONES).trim()) throw new Error('Para completar la actividad indicá una evidencia o comentario de cierre.');
}
function activityRecordFromInput_(input, current, founders, periods, user, isNew) {
  ...
  record.FECHA_INICIO = activityDate_(get('startDate', record.FECHA_INICIO)); record.FECHA_VENCIMIENTO = activityDate_(get('dueDate', record.FECHA_VENCIMIENTO)); record.PUNTOS = activityNumber_(get('points', record.PUNTOS)); record.PRIORIDAD = activityText_(get('priority', record.PRIORIDAD || 'Media')); record.ESTADO = activityText_(get('status', record.ESTADO || 'Sin asignar')); record.PORCENTAJE_AVANCE = activityNumber_(get('progress', record.PORCENTAJE_AVANCE)); record.EVIDENCIA = activityText_(get('evidence', record.EVIDENCIA)); record.OBSERVACIONES = activityText_(get('notes', record.OBSERVACIONES)); record.ORDEN_TABLERO = activityNumber_(get('boardOrder', record.ORDEN_TABLERO || Date.now()));
  if (!record.ID_RESPONSABLE) record.ESTADO = 'Sin asignar'; else if (record.ESTADO === 'Sin asignar') record.ESTADO = 'Pendiente';
  if (record.ESTADO === 'Completada') { record.PORCENTAJE_AVANCE = 100; record.FECHA_FINALIZACION = activityDate_(get('finishedAt', record.FECHA_FINALIZACION)) || now; record.FACTOR_CUMPLIMIENTO = 1; } else { record.FACTOR_CUMPLIMIENTO = record.ESTADO === 'No cumplida' ? 0 : 0; if (record.ESTADO !== 'Completada') record.FECHA_FINALIZACION = ''; }
  record.PUNTOS_OBTENIDOS = activityNumber_(record.PUNTOS) * activityNumber_(record.FACTOR_CUMPLIMIENTO); record.MODIFICADA_POR = user; record.FECHA_MODIFICACION = now; record.ACTIVA = source.active === false ? 'No' : (current ? (activityYes_(current.ACTIVA) ? 'Sí' : 'No') : 'Sí');
...
function activityCreatePeriod_(body) {
  ...
  try {
    ...
    const record = { ID_PERIODO: id, NOMBRE: name, DESCRIPCION: activityText_(input.description), FECHA_INICIO: start, FECHA_FIN: end, TOTAL_PUNTOS_OBJETIVO: activityNumber_(input.targetPoints, 100), ESTADO: status, FECHA_CIERRE: '', CERRADO_POR: '', OBSERVACIONES: activityText_(input.notes), FECHA_CREACION: activityNow_(), CREADO_POR: user }; activityAppend_(sheets.periods, record); if (status === 'Activo') { const configRows = activityRecords_(sheets.config); const index = configRows.findIndex((row) => activityText_(row.CLAVE) === 'PERIODO_ACTIVO'); if (index >= 0) { const row = configRows[index]; row.VALOR = id; activityWriteRow_(sheets.config, index + 2, row); } } return activitySuccess_(activityPublicPeriod_(record), 'Período creado.');
  } finally { lock.releaseLock(); }
}

function activityUpdatePeriod_(body) {
  const lock = LockService.getScriptLock(); lock.waitLock(10000);
  try {
    const sheets = activitySheetsUnlocked_(); const input = body.period || {}; const id = activityText_(input.id); const row = activityFindRow_(sheets.periods, 'ID_PERIODO', id); if (row < 2) throw new Error('No se encontró el período a editar.'); const name = activityText_(input.name).trim(); if (!name) throw new Error('El nombre del período es obligatorio.'); const record = activityReadRow_(sheets.periods, row); record.NOMBRE = name; activityWriteRow_(sheets.periods, row, record); return activitySuccess_(activityPublicPeriod_(record), 'Período actualizado.');
  } finally { lock.releaseLock(); }
}
...
function activityDeletePeriod_(body) {
  const lock = LockService.getScriptLock(); lock.waitLock(10000);
  try {
    const sheets = activitySheetsUnlocked_(); const id = activityText_(body.periodId); const row = activityFindRow_(sheets.periods, 'ID_PERIODO', id); if (row < 2) throw new Error('No se encontró el período a eliminar.'); if (activityRecords_(sheets.activities).some((activity) => activityText_(activity.ID_PERIODO) === id)) throw new Error('No se puede eliminar un período que tiene actividades. Reasigná o eliminá sus actividades primero.'); sheets.periods.deleteRow(row); const configRows = activityRecords_(sheets.config); const configIndex = configRows.findIndex((record) => activityText_(record.CLAVE) === 'PERIODO_ACTIVO'); if (configIndex >= 0 && activityText_(configRows[configIndex].VALOR) === id) { const active = activityRecords_(sheets.periods).find((period) => activityText_(period.ESTADO) === 'Activo'); configRows[configIndex].VALOR = active ? activityText_(active.ID_PERIODO) : ''; activityWriteRow_(sheets.config, configIndex + 2, configRows[configIndex]); } return activitySuccess_({ id: id }, 'Período eliminado.');
  } finally { lock.releaseLock(); }
}
...
function activityDelete_(body, preparedSheets) {
  const lock = LockService.getScriptLock(); lock.waitLock(10000);
  try {
    const sheets = preparedSheets || activitySheets_(); const id = activityText_(body.activityId || body.id); const headers = activityHeaderMap_(sheets.activities);
    const row = activityFindRowWithHeaders_(sheets.activities, headers, 'ID_ACTIVIDAD', id);
    if (row < 2) throw new Error('No se encontro la actividad a eliminar.');
    sheets.activities.deleteRow(row);
    activityClearResponsibilitiesCache_();
    return activitySuccess_({ id: id }, 'Actividad eliminada.');
  } finally { lock.releaseLock(); }
}
...
function handleActivitiesPost_(body) {
  try {
    ...
    if (action === 'deleteActivity') return activityDelete_(body, sheets);
