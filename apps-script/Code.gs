...
function activitySheets_() { crearHojasActividades(); const spreadsheet = getActivitiesSpreadsheet_(); return { founders: spreadsheet.getSheetByName(ACTIVITY_SHEETS_.founders), periods: spreadsheet.getSheetByName(ACTIVITY_SHEETS_.periods), activities: spreadsheet.getSheetByName(ACTIVITY_SHEETS_.activities), history: spreadsheet.getSheetByName(ACTIVITY_SHEETS_.history), config: spreadsheet.getSheetByName(ACTIVITY_SHEETS_.config) }; }
...
function obtenerResumenActividades(idPeriodo) { const activities = obtenerActividades().filter((activity) => !idPeriodo || activity.periodId === idPeriodo); const founders = obtenerFundadoras(); return founders.map((founder) => { const assigned = activities.filter((activity) => activity.responsibleId === founder.id && activity.active && activity.status !== 'Cancelada'); const assignedPoints = assigned.reduce((sum, activity) => sum + activity.points, 0); const obtainedPoints = assigned.reduce((sum, activity) => sum + activity.pointsObtained, 0); return { founderId: founder.id, founder: founder.displayName, assigned: assigned.length, completed: assigned.filter((activity) => activity.status === 'Completada').length, pending: assigned.filter((activity) => !['Completada', 'Cancelada'].includes(activity.status)).length, assignedPoints: assignedPoints, obtainedPoints: obtainedPoints, compliancePercent: assignedPoints ? (obtainedPoints / assignedPoints) * 100 : 0 }; }); }
