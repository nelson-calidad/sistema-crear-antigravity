...
function CoveragePrintSheet({ week, shifts, nameFor }: { week: Date; shifts: CoverageShift[]; nameFor: (id?: string) => string }) {
  ...
  const PrintDay = ({ day }: { day: Date; key?: string }) => { const entries = shifts.filter((shift) => shift.date === dateKey(day)); return <article className="coverage-print-day"><header><p>{dayName(day)}</p><strong>{dateLabel(day)}</strong></header><div className="coverage-print-day-content">{entries.length ? entries.map((shift) => <div key={shift.id} className="coverage-print-card"><p className="coverage-print-time">{shift.startTime} a {shift.endTime}</p><p className="coverage-print-person">{nameFor(shift.primaryId)}{shift.secondaryId ? ` + ${nameFor(shift.secondaryId)}` : ''}</p><p className="coverage-print-detail">{shift.type === 'control' ? `Control - ${shift.professional || shift.place}` : shift.place}</p><span>{statusLabel(shift.status)}</span></div>) : <p className="coverage-print-empty">Sin cobertura programada</p>}</div></article>; };
  const hasWeekend = days.slice(5).some((day) => shifts.some((shift) => shift.date === dateKey(day)));
  return <section className="coverage-print-sheet" aria-hidden="true"><header className="coverage-print-header"><div><p>CREAR | Espacio Terapeutico</p><h1>Planilla semanal de guardias y controles</h1></div><div><strong>Semana: {weekLabel(week)}</strong><span>Emitido el {issuedAt}</span></div></header><p className="coverage-print-intro">Responsables, horarios y controles programados para cada dia de la semana.</p><div className="coverage-print-grid">{days.slice(0, 5).map((day) => <PrintDay key={dateKey(day)} day={day} />)}</div>{hasWeekend && <section className="coverage-print-weekend"><p>Fin de semana</p><div>{days.slice(5).map((day) => <PrintDay key={dateKey(day)} day={day} />)}</div></section>}<p className="coverage-print-footnote">Este documento resume la programacion operativa de CREAR para la semana indicada.</p></section>;
}