...
function CoverageShiftPreview({ shift, weeklyNote, nameFor, onClose, onEdit, onDelete, saving }: { shift: CoverageShift; weeklyNote?: WeeklyCoverageNote; nameFor: (id?: string) => string; onClose: () => void; onEdit: () => void; onDelete: () => void; saving: boolean }) {
  const tasks = weeklyNote?.tasks || [];
  const statusLabel = shift.status === 'Completed' ? 'Realizada' : shift.status === 'Rescheduled' ? 'Reprogramada' : shift.status === 'Cancelled' ? 'No realizada' : 'Planificada';
  return <div className="fixed inset-0 z-[80] flex items-end justify-center bg-slate-950/40 sm:items-center sm:p-5" role="dialog" aria-modal="true" aria-label="Resumen de guardia">
    <section className="w-full max-w-md overflow-hidden rounded-t-3xl bg-white shadow-2xl dark:bg-slate-900 sm:rounded-3xl">
      <header className="flex items-start justify-between border-b border-slate-100 p-5 dark:border-slate-800"><div><p className="text-[10px] font-black uppercase tracking-[.2em] text-sky-600">Resumen de guardia</p><h2 className="mt-1 text-xl font-bold">{shift.place || 'Guardia CREAR'}</h2><p className="mt-1 text-sm font-medium text-slate-500">{new Intl.DateTimeFormat('es-AR', { weekday: 'long', day: 'numeric', month: 'long' }).format(parseDate(shift.date))}</p></div><button type="button" onClick={onClose} className="rounded-xl p-2 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800" aria-label="Cerrar"><X className="h-5 w-5" /></button></header>
      <div className="space-y-4 p-5"><div className="grid grid-cols-2 gap-3"><div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-800"><p className="text-[10px] font-black uppercase text-slate-400">Horario</p><p className="mt-1 text-sm font-black">{shift.startTime} a {shift.endTime}</p></div><div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-800"><p className="text-[10px] font-black uppercase text-slate-400">Estado</p><p className="mt-1 text-sm font-black text-sky-700 dark:text-sky-300">{statusLabel}</p></div></div><div><p className="text-[10px] font-black uppercase tracking-[.14em] text-slate-400">Responsables</p><p className="mt-1 text-sm font-bold text-slate-700 dark:text-slate-200">{nameFor(shift.primaryId)}{shift.secondaryId ? ` + ${nameFor(shift.secondaryId)}` : ''}</p></div>{shift.notes && <div><p className="text-[10px] font-black uppercase tracking-[.14em] text-slate-400">Registro / novedad</p><p className="mt-1 rounded-xl bg-slate-50 p-3 text-sm font-medium text-slate-600 dark:bg-slate-800 dark:text-slate-300">{shift.notes}</p></div>}{shift.hasWeeklyNote && <section className="rounded-2xl border border-amber-200 bg-amber-50/70 p-3 dark:border-amber-900/70 dark:bg-amber-950/20"><div className="flex items-center gap-2"><StickyNote className="h-4 w-4 text-amber-600" /><div><p className="text-xs font-black text-amber-900 dark:text-amber-200">Tareas adjuntas de la semana</p><p className="text-[11px] font-medium text-slate-500 dark:text-slate-400">Vista de consulta</p></div></div>{tasks.length ? <ol className="mt-3 space-y-1.5">{tasks.map((task, index) => <li key={task.id} className="flex items-center gap-2 rounded-lg bg-white/70 px-2.5 py-2 text-xs dark:bg-slate-900/60"><CheckCircle2 className={cn('h-3.5 w-3.5 shrink-0', task.completed ? 'text-emerald-600' : 'text-slate-300')} /><span className="font-black text-amber-700">{index + 1}</span><span className={cn('font-semibold', task.completed && 'text-slate-400 line-through')}>{task.text}</span></li>)}</ol> : <p className="mt-3 rounded-lg bg-white/60 px-3 py-2 text-xs font-medium text-slate-500 dark:bg-slate-900/50">Todavía no hay tareas cargadas para esta semana.</p>}</section>}</div>
      <footer className="flex flex-col gap-2 border-t border-slate-100 p-4 dark:border-slate-800"><button type="button" disabled={saving} onClick={onEdit} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-60 dark:bg-slate-100 dark:text-slate-900"><Pencil className="h-4 w-4" /> Editar actividad</button><div className="flex gap-2"><button type="button" disabled={saving} onClick={onDelete} className="inline-flex flex-1 items-center justify-center gap-1 rounded-xl px-3 py-2 text-xs font-bold text-rose-600 hover:bg-rose-50 disabled:opacity-60 dark:hover:bg-rose-950/30"><Trash2 className="h-4 w-4" /> Eliminar</button><button type="button" onClick={onClose} className="flex-1 rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold text-slate-600 dark:border-slate-700 dark:text-slate-300">Cerrar</button></div></footer>
    </section>
  </div>;
}
  ...
  return <div className="fixed inset-0 z-[80] flex items-end justify-center bg-slate-950/40 sm:items-center sm:p-5" role="dialog" aria-modal="true">
    <form onSubmit={submit} className="max-h-[94dvh] w-full max-w-2xl overflow-y-auto rounded-t-3xl bg-white shadow-2xl dark:bg-slate-900 sm:rounded-3xl">
      ...
      <div className="grid gap-4 p-5 sm:grid-cols-2">
        ...
        <label className="text-xs font-bold text-slate-600 dark:text-slate-300">Estado<select value={status} onChange={(event) => setStatus(event.target.value as CoverageShift['status'])} className={inputClass}><option value="Planned">Planificada</option><option value="Completed">Realizada</option><option value="Rescheduled">Reprogramada</option><option value="Cancelled">No realizada</option></select><span className="mt-1 block text-[10px] font-medium text-slate-400">Solo las realizadas suman horas.</span></label>
...
export const CoveragePlanner = () => {
  ...
  return <>
    ...
    {preview && <CoverageShiftPreview shift={preview} weeklyNote={preview.hasWeeklyNote ? weeklyNotes.find((note) => note.weekStart === dateKey(monday(parseDate(preview.date)))) : undefined} nameFor={nameFor} onClose={() => setPreview(null)} onEdit={() => { setEditor(preview); setPreview(null); }} onDelete={() => { setPreview(null); void remove(preview); }} saving={saving} />}
    </div>
  </>;
};
function CoveragePrintSheet({ month, shifts, nameFor }: { month: Date; shifts: CoverageShift[]; nameFor: (id?: string) => string }) {
  ...
  const statusLabel = (status: CoverageShift['status']) => status === 'Completed' ? 'Realizada' : status === 'Rescheduled' ? 'Reprogramada' : status === 'Cancelled' ? 'No realizada' : 'Planificada';
