...
function PeriodForm({ onClose, onSave, saving }: { onClose: () => void; onSave: (period: Partial<ActivityPeriod>) => Promise<void>; saving: boolean }) {
  ...
  return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/35 p-4"><form onSubmit={submit} className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl dark:bg-slate-900"><div className="flex items-center justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-slate-400">Organizacion</p><h2 className="text-lg font-bold">Nuevo periodo</h2></div><button type="button" onClick={onClose} className="rounded-xl p-2 text-slate-400" aria-label="Cerrar"><X className="h-5 w-5" /></button></div><label className="mt-5 block text-xs font-bold text-slate-600 dark:text-slate-300">Nombre<input autoFocus value={name} onChange={(event) => setName(event.target.value)} className={inputClass} placeholder="Ej. Apertura del centro" /></label>{error && <p className="mt-3 text-xs font-medium text-rose-600">{error}</p>}<div className="mt-6 flex justify-end gap-3"><button type="button" onClick={onClose} className="rounded-xl px-4 py-2.5 text-sm font-bold text-slate-500">Cancelar</button><button disabled={saving} className="rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-bold text-white dark:bg-slate-100 dark:text-slate-900">{saving ? 'Guardando...' : 'Crear periodo'}</button></div></form></div>;
}
...
export const Responsibilities = () => {
  ...
  const savePeriod = async (period: Partial<ActivityPeriod>) => {
    setSaving(true);
    try { const saved = await createPeriod(period, CURRENT_USER); setPeriods((current) => [...current, saved]); setPeriodId(saved.id); setShowPeriodForm(false); setNotice({ tone: 'success', message: 'Periodo creado.' }); } catch (cause) { setNotice({ tone: 'error', message: cause instanceof Error ? cause.message : 'No se pudo crear el periodo.' }); } finally { setSaving(false); }
  };
  const completeActivity = async (activity: ActivityRecord) => {
    if (activity.status !== 'Completada') await save({ ...activity, status: 'Completada', progress: 100, finishedAt: dateKey(new Date()), evidence: activity.evidence || 'Confirmada como realizada desde el tablero.' }, 'Marcada como completada');
  };
  ...
  return <div className="space-y-5">
    ...
    <header className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.22em] text-slate-400">Planificacion operativa</p><button onClick={() => setShowCalendar(true)} className="group mt-1 inline-flex items-center gap-2 rounded-lg text-left" title="Abrir calendario de actividades"><h1 className="text-2xl font-bold text-slate-900 transition group-hover:text-sky-700 dark:text-slate-100 md:text-3xl">Responsabilidades</h1><CalendarDays className="h-5 w-5 text-sky-600" /></button><p className="mt-1 text-sm font-medium text-slate-500 dark:text-slate-400">Organiza, asigna y completa las tareas. Toca el titulo para ver las fechas.</p></div><div className="flex flex-wrap gap-2"><button onClick={() => setShowCalendar(true)} disabled={!periodId} className="inline-flex items-center gap-2 rounded-xl border border-sky-200 bg-sky-50 px-4 py-2.5 text-sm font-bold text-sky-700 disabled:opacity-50"><CalendarDays className="h-4 w-4" /> Calendario</button><button onClick={() => window.print()} disabled={!periodId} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 disabled:opacity-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200"><Printer className="h-4 w-4" /> Imprimir actas</button><button onClick={() => setShowPeriodForm(true)} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200"><CalendarDays className="h-4 w-4" /> Nuevo periodo</button><button onClick={() => setEditing(null)} disabled={!periodId} className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-bold text-white disabled:opacity-50 dark:bg-slate-100 dark:text-slate-900"><Plus className="h-4 w-4" /> Nueva actividad</button></div></header>
    ...
    {!error && <><div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:flex-row"><div className="relative flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={search} onChange={(event) => setSearch(event.target.value)} className="w-full rounded-xl bg-slate-50 py-2.5 pl-10 pr-3 text-sm outline-none dark:bg-slate-800" placeholder="Buscar actividad" /></div><select value={periodId} onChange={(event) => setPeriodId(event.target.value)} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200">{periods.map((period) => <option key={period.id} value={period.id}>{period.name}</option>)}</select></div>
