...
const activitySignal = (activity: ActivityRecord) => {
  ...
  if (activity.status === 'Bloqueada' || activity.status === 'No cumplida') return { color: 'bg-rose-500', label: activity.status === 'No cumplida' ? 'No realizada' : 'Bloqueada', badge: 'bg-rose-50 text-rose-700 dark:bg-rose-950/40 dark:text-rose-300' };
...
type CompletionResult = 'completed' | 'notCompleted' | 'pending';

function CompletionNoteForm({ activity, onClose, onSave, saving }: { activity: ActivityRecord; onClose: () => void; onSave: (note: string, result: CompletionResult) => Promise<void>; saving: boolean }) {
  const [result, setResult] = useState<CompletionResult>(activity.status === 'Completada' ? 'pending' : activity.status === 'No cumplida' ? 'notCompleted' : 'completed');
  ...
  const submit = async (event: FormEvent) => {
    event.preventDefault();
    if (!isPending && !note.trim()) return setError(isCompleted ? 'Escribi que se realizo o una acotacion.' : 'Escribi el motivo por el que no se realizo.');
    await onSave(note.trim(), result);
  };
  return <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/45 p-4" role="dialog" aria-modal="true">
    <form onSubmit={submit} className="w-full max-w-lg rounded-3xl bg-white shadow-2xl dark:bg-slate-900">
      ...
      <div className="space-y-4 p-5">
        <label className="block text-xs font-bold text-slate-600 dark:text-slate-300">Resultado<select value={result} onChange={(event) => setResult(event.target.value as CompletionResult)} className={inputClass}><option value="completed">Realizada</option><option value="notCompleted">No realizada</option><option value="pending">Pendiente</option></select></label>
        <label className="block text-xs font-bold text-slate-600 dark:text-slate-300">{isPending ? 'Acotacion (opcional)' : isCompleted ? 'Que se hizo / observacion' : 'Motivo o impedimento'}<textarea autoFocus value={note} onChange={(event) => setNote(event.target.value)} className={cn(inputClass, 'min-h-28 resize-y')} placeholder={isPending ? 'Ej. Se vuelve a pendiente porque se marco por error.' : isCompleted ? 'Ej. Se presento la documentacion en la municipalidad.' : 'Ej. No se pudo realizar porque faltaba documentacion.'} /></label>
        {error && <p className="rounded-xl bg-rose-50 px-3 py-2 text-xs font-medium text-rose-700">{error}</p>}
      </div>
      <footer className="flex justify-end gap-3 border-t border-slate-100 p-5 dark:border-slate-800"><button type="button" onClick={onClose} disabled={saving} className="rounded-xl px-4 py-2.5 text-sm font-bold text-slate-500">Cancelar</button><button disabled={saving} className={cn('inline-flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-bold text-white disabled:opacity-60', isCompleted ? 'bg-slate-900 dark:bg-slate-100 dark:text-slate-900' : isPending ? 'bg-amber-500' : 'bg-rose-600')}>{saving && <LoaderCircle className="h-4 w-4 animate-spin" />}{isCompleted ? 'Guardar como realizada' : isPending ? 'Volver a pendiente' : 'Guardar como no realizada'}</button></footer>
    </form>
  </div>;
}
...
export const Responsibilities = () => {
  ...
  const confirmActivityCompletion = async (note: string, result: CompletionResult) => {
    const activity = completionRequest;
    if (!activity) return;
    setCompletionRequest(null);
    if (result === 'completed') await save({ ...activity, status: 'Completada', progress: 100, finishedAt: dateKey(new Date()), evidence: note, notes: note }, `Actividad realizada: ${note}`);
    else if (result === 'notCompleted') await save({ ...activity, status: 'No cumplida', progress: 0, finishedAt: undefined, evidence: undefined, notes: note }, `Actividad no realizada: ${note}`);
    else await save({ ...activity, status: 'Pendiente', progress: 0, finishedAt: undefined, evidence: undefined, notes: note || undefined }, note ? `Actividad devuelta a pendiente: ${note}` : 'Actividad devuelta a pendiente');
  };
  ...
  const commitmentActivities = followupActivities.filter((activity) => Boolean(activity.responsibleId));
  const commitmentPrintSheet = (
    <section id="responsibilities-commitment-print" className="responsibilities-commitment-print-sheet" aria-hidden="true">
      <article className="responsibilities-commitment-document">
        <header className="responsibilities-commitment-header"><div><p>CREAR · Espacio Terapeutico</p><h1>Acta de compromiso operativo</h1><span>Asignacion de responsabilidades del periodo</span></div><div><strong>{activePeriod?.name || 'Periodo actual'}</strong><span>Emitido el {printedDate}</span></div></header>
        <section className="responsibilities-commitment-summary"><div><span>Periodo</span><strong>{activePeriod?.name || 'Actual'}</strong></div><div><span>Responsables</span><strong>{founders.length}</strong></div><div><span>Actividades asignadas</span><strong>{commitmentActivities.length}</strong></div></section>
        <p className="responsibilities-commitment-text">Las personas abajo firmantes declaran haber recibido las responsabilidades detalladas en esta acta y se comprometen a ejecutarlas con diligencia dentro de los plazos acordados. Ante cualquier impedimento, demora o necesidad de reprogramacion, se comprometen a informarlo oportunamente a CREAR.</p>
        <table className="responsibilities-commitment-table"><thead><tr><th>Responsable</th><th>Actividad comprometida</th><th>Inicio</th><th>Fecha limite</th></tr></thead><tbody>{commitmentActivities.length ? commitmentActivities.map((activity) => <tr key={activity.id}><td>{founderById.get(activity.responsibleId || '')?.displayName || 'Sin asignar'}</td><td>{activity.title}</td><td>{formatActivityDate(activity.startDate, true)}</td><td>{formatActivityDate(activity.dueDate, true)}</td></tr>) : <tr><td colSpan={4} className="is-empty">No registra actividades asignadas en este periodo.</td></tr>}</tbody></table>
        <p className="responsibilities-commitment-footnote">La presente acta constituye constancia interna de la asignacion de tareas y del compromiso asumido por las responsables identificadas.</p>
        <footer className="responsibilities-commitment-signatures">{founders.map((founder) => <div key={founder.id}><strong>{founder.displayName}</strong><span>Firma y aclaracion</span></div>)}</footer>
      </article>
    </section>
  );
  ...
  return <>
    ...
    {!error && <><div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:flex-row"><div className="relative flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={search} onChange={(event) => setSearch(event.target.value)} className="w-full rounded-xl bg-slate-50 py-2.5 pl-10 pr-3 text-sm outline-none dark:bg-slate-800" placeholder="Buscar actividad" /></div><div className="flex gap-2"><select value={periodId} onChange={(event) => setPeriodId(event.target.value)} className="min-w-0 flex-1 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200">{periods.map((period) => <option key={period.id} value={period.id}>{period.name}</option>)}</select><button onClick={() => activePeriod && setPeriodEditor(activePeriod)} disabled={!activePeriod} className="rounded-xl border border-slate-200 px-3 text-slate-600 disabled:opacity-40 dark:border-slate-700 dark:text-slate-300" title="Editar nombre del periodo"><Pencil className="h-4 w-4" /></button><button onClick={() => activePeriod && setPeriodToDelete(activePeriod)} disabled={!activePeriod} className="rounded-xl border border-rose-200 px-3 text-rose-600 disabled:opacity-40" title="Eliminar periodo"><Trash2 className="h-4 w-4" /></button></div></div>
      ...
      {completionRequest && <CompletionNoteForm activity={completionRequest} onClose={() => setCompletionRequest(null)} onSave={confirmActivityCompletion} saving={saving} />}
